...
copy_snapshot () {
    ...
    copy_snapshot_id=`aws ec2 copy-snapshot \
        ...
        --description "This snapshot is a copy from $last_snapshot in region $SOURCE_REGION."\
        --query="SnapshotId"\
        --output text`
    aws ec2 create-tags \
    ...
    echo $(date '+%Y-%m-%d %H:%M:%S:%N') "Snapshot ID from new copied snapshot in region $TARGET_REGION = $copy_snapshot_id" | tee -a $LOG_PATH/$LOG_FILE
    ...
    for i in `seq 1 10`; do
       status=`aws ec2 describe-snapshots \
        ...
        if [[ $status == "completed" ]]; then
            echo $(date '+%Y-%m-%d %H:%M:%S:%N') "Snapshot migration finished successfully." | tee -a $LOG_PATH/$LOG_FILE
            break
        else
            echo $(date '+%Y-%m-%d %H:%M:%S:%N') "Snapshot migration in $status state." | tee -a $LOG_PATH/$LOG_FILE
        fi
        if [[ $i == 10 ]]; then
            echo $(date '+%Y-%m-%d %H:%M:%S:%N') "Timeout. Migration of snapshot takes to long." | tee -a $LOG_PATH/$LOG_FILE
            exit 1    
        fi
...
remove_snapshot_different_than () {
    ...
    for o in $old_snapshots; do
        ...
        response=`aws ec2 delete-snapshot\
            ...
            --region $TARGET_REGION`
        if [[ $response == "" ]]; then
            echo $(date '+%Y-%m-%d %H:%M:%S:%N') "Snapshot $o removed successfully." | tee -a $LOG_PATH/$LOG_FILE
        fi
    done
}
...
# Main
echo $(date '+%Y-%m-%d %H:%M:%S:%N') "Getting snapshot ID from volume $VOLUME_ID in region $SOURCE_REGION." | tee -a $LOG_PATH/$LOG_FILE
...
echo $(date '+%Y-%m-%d %H:%M:%S:%N') "Coping snapshot ID $last_snapshot to region $TARGET_REGION." | tee -a $LOG_PATH/$LOG_FILE
copy_snapshot $last_snapshot

echo $(date '+%Y-%m-%d %H:%M:%S:%N') "Removing snapshots for volume $VOLUME_ID in region $TARGET_REGION differents than $copy_snapshot_id ." | tee -a $LOG_PATH/$LOG_FILE
